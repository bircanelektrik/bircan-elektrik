...
(function () {
  ...
  /* ─────────────────────────────────────────
     6. HİZMETLER — güvenli kart görünürlüğü
     Kartlar varsayılan olarak görünür kalsın; animasyon sadece gerektiğinde çalışsın.
  ───────────────────────────────────────── */
  (function () {
    ...
    if (grid.getBoundingClientRect().top < window.innerHeight * 0.92) {
      requestAnimationFrame(revealServiceCards);
      return;
    }
  ...
  ScrollTrigger.create({
    ...
    onEnter: function () {
      ...
      gsap.fromTo(cards,
        ...
        { opacity: 1, scale: 1, stagger: 0.04, duration: 0.55, ease: 'power2.out', clearProps: 'transform,opacity' }
      );
    },
    once: true
  });
    ...
    cards.forEach(function (card) {
      card.addEventListener('mouseenter', function () {
        ...
        cards.forEach(function (other) {
          other.classList.toggle('is-hovered', other === card);
        });
      });
    });
